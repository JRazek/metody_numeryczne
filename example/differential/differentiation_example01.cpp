...
auto main() -> int {
  ...
  auto res1 = partialDerivative<0, f>(funciton, 2 * std::numbers::pi, 2.0);

  auto res2 = partialDerivative<0, d>(funciton, 2 * std::numbers::pi, 2.0);

  auto res3 = partialDerivative<0, ld>(funciton, 2 * std::numbers::pi, 2.0);
